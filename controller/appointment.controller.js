...
const auth_token = '96770bb2506a1e363fe618d6d9fa6495';
...
const cancelAppointment = async (request, response) => {
    ...
    const name = patient.patientName;
    try {
        const data = await Appointment.findByIdAndDelete({ _id });
        if (!data) {
...
const acceptAppointments = async (request, response) => {
    ...
    try {
        const appointment = await Appointment.findByIdAndUpdate({ _id }, { $set: data }, { new: true });
        if (!appointment) {
            const error = new HttpError(404, "Appointment not Found!");
            console.log("error: ", error);
            return { error };
